...
public class BluetoothDeviceList extends Activity {
	...
	private OnCreateContextMenuListener mCreateContextMenuListener = new OnCreateContextMenuListener(){
		...
		public void onCreateContextMenu(ContextMenu menu, View view,ContextMenuInfo arg2) {
			menu.setHeaderTitle("select_options");
			...
			if (info.contains(" ( " +"has_paired" +" )")) {
				menu.add(0, 0, 0, "rePaire_connect").setOnMenuItemClickListener(mOnMenuItemClickListener);
				menu.add(0, 1, 1, "connect_paired").setOnMenuItemClickListener(mOnMenuItemClickListener);
			}
			else
			{
				menu.add(0, 2, 2, "paire_connect").setOnMenuItemClickListener(mOnMenuItemClickListener);
			}
		}
    };
